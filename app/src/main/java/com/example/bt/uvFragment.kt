...
class uvFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        viewh = inflater.inflate(R.layout.fragment_uv2, container, false)

        val repository = DataRepository(SensorDataDatabase.getDatabase(requireActivity().application).dataDao())
        repository.readAllUvData.observe(viewLifecycleOwner,{
            var helper = 1
            uiScope.launch {
                withContext(Dispatchers.Main){
                    if(fetchFirstData == false) {
                        it.forEach{ data ->
                            ...
                            lineentry.add(Entry(data.uvData.subSequence(1,5).toString().toFloat(), helper))
helper++
    ...
    fun setLineChartData() {
        val ez = viewh?.findViewById<LineChart>(R.id.linechart3)
        Log.i("test","problem1")
//        val xvalue = ArrayList<String>()
//        xvalue.add("11:00 AM")
//        xvalue.add("12:00 AM")
...
//        lineentry.add(Entry(32f, 8))
        ...
        val linedataset = LineDataSet(lineentry, "Uv")
        Log.i("test","problem2")
